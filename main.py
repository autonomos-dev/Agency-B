...
def run_logic(brand_task, user_task):
    ...
    # Definición de los agentes dentro de run_logic
    ...
    # Aquí se pueden agregar los mensajes generados a la lista
    for message in groupchat.messages:
        # Asegúrate de que el mensaje tenga las claves correctas
        if 'sender' in message and 'content' in message:
            messages.append(f"{message['sender']}: {message['content']}")
        else:
            # Manejo de errores si las claves no están presentes
            messages.append(f"Mensaje no estructurado: {message}")

    # Devuelve todos los mensajes generados
    return {
        "status": "success",
        "message": "Lógica ejecutada correctamente",
        "all_messages": "<br>".join(messages)  # Devuelve todos los mensajes generados
    }